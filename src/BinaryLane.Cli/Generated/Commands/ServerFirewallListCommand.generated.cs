...
public static class ServerFirewallListCommand
{
    public static Command Create()
    {
        var command = new Command("list", "Fetch all advanced firewall rules for a server");

        var server_idArg = new Argument<string>("server_id") { Description = "The ID of the server for which firewall rules should be listed." };
        ...
        command.SetAction(async (parseResult, ct) =>
        {
            ...
            return await ApiCommandBase.ExecuteApiCallAsync(
                cmdCtx,
                async client => await client.GetAsync($"v2/servers/{server_id}/advanced_firewall_rules", ct),
                json =>
                {
