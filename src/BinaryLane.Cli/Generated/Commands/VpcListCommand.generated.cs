...
public static class VpcListCommand
{
    public static Command Create()
    {
        var command = new Command("list", "List all existing VPCs");

        command.SetAction(async (parseResult, ct) =>
        {
            var cmdCtx = ContextBinder.Bind(parseResult);

            string[] defaultFields = ["id", "name", "ip_range"];
            ...
            return await ApiCommandBase.ExecuteApiCallAsync(
                cmdCtx,
                client => ApiCommandBase.ExecutePaginatedAsync(client, $"v2/vpcs"),
                json =>
                {
